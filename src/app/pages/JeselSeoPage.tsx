...
export default function JeselSeoPage() {
  ...
  const title = route?.title ?? "Virtual Assistant & Operations Support | SageStone Inc";
  const description = route?.description ?? "Virtual assistant services and business operations support for growing teams.";
  ...
  const crumbs = useMemo(() => {
    ...
    if (path !== "/") base.push({ name: title.replace(/ \| SageStone Inc$/, ""), item: canonicalUrl(path) });
    return base;
  ...
  useEffect(() => {
    ...
    if (route?.kind === "blog") setJsonLd("article-jsonld", { "@context": "https://schema.org", "@type": "BlogPosting", headline: title.replace(/ \| SageStone Inc$/, ""), description, author: { "@type": "Organization", name: SITE_NAME, url: SITE_URL }, publisher: { "@type": "Organization", name: SITE_NAME, url: SITE_URL }, mainEntityOfPage: canonicalUrl(path), datePublished: "2026-06-30", dateModified: "2026-07-05", image: OG_IMAGE, url: canonicalUrl(path) });
    else document.getElementById("article-jsonld")?.remove();
    if (path === "/about") setJsonLd("about-page-jsonld", { "@context": "https://schema.org", "@type": "AboutPage", name: title.replace(/ \| SageStone Inc$/, ""), description, url: canonicalUrl(path), mainEntity: { "@type": "Organization", name: SITE_NAME, url: SITE_URL, logo: OG_IMAGE } });
    else document.getElementById("about-page-jsonld")?.remove();
  }, [crumbs, description, path, route?.kind, title]);

  const answer = answers[path];
  return <main className="min-h-screen bg-[color:var(--brand-cloud)]"><section className="mx-auto max-w-5xl px-4 py-20 sm:px-6 lg:px-8"><nav className="mb-8 text-sm text-black/55">{crumbs.map((c, i) => <span key={c.item}>{i > 0 && " / "}<Link className="underline underline-offset-4" to={new URL(c.item).pathname}>{c.name}</Link></span>)}</nav><p className="mb-4 text-xs font-bold uppercase tracking-[0.22em] text-[color:var(--brand-deep-sage)]">Operations support</p><h1 className="max-w-4xl text-balance text-[clamp(2.4rem,7vw,4.5rem)] font-[760] leading-none text-[color:var(--brand-charcoal)]">{title.replace(/ \| SageStone Inc$/, "")}</h1><p className="mt-6 max-w-3xl text-lg leading-8 text-black/68">{description}</p>{answer && <section className="mt-10 rounded-[28px] border border-[color:var(--border)] bg-white/80 p-7 shadow-[0_16px_46px_rgba(35,81,59,0.08)]"><h2 className="text-2xl font-semibold text-[color:var(--brand-charcoal)]">{answer.q}</h2><p className="mt-4 text-base leading-8 text-black/68">{answer.a}</p></section>}<section className="mt-10 grid gap-4 md:grid-cols-3">{["Workflow clarity", "SOPs and CRM hygiene", "Delegation readiness"].map((item) => <article key={item} className="rounded-3xl border border-[color:var(--border)] bg-white/70 p-6"><h2 className="text-xl font-semibold text-[color:var(--brand-charcoal)]">{item}</h2><p className="mt-3 text-sm leading-7 text-black/60">Practical, documented improvements that help teams serve customers consistently and reduce operational drag.</p></article>)}</section><div className="mt-10"><Link to="/contact" className="inline-flex rounded-full bg-[color:var(--brand-deep-sage)] px-6 py-3 text-sm font-semibold text-white">Discuss your workflow</Link></div></section></main>;
}
