...
export default function Home() {
  ...
  return <>
    <section className="relative isolate overflow-hidden pt-8" style={{ background: "radial-gradient(circle at 80% 12%, rgba(221,234,215,.82), transparent 30rem), radial-gradient(circle at 12% 42%, rgba(239,228,209,.72), transparent 26rem), linear-gradient(135deg, var(--brand-cloud), var(--brand-ivory) 58%, #eef4e9)" }}><div className="mx-auto grid max-w-[1440px] items-center gap-12 px-4 py-16 sm:px-6 lg:grid-cols-[1.02fr_0.98fr] lg:px-8 lg:py-24"><div className="reveal-up"><p className="mb-5 inline-flex rounded-full pill-glow border border-[color:var(--brand-deep-sage)]/16 bg-white/72 px-4 py-2 text-xs font-bold uppercase tracking-[0.2em] text-[color:var(--brand-deep-sage)]">Premium operations partner</p><h1 className="max-w-5xl text-balance text-[color:var(--brand-charcoal)]" style={{ fontSize: "clamp(2.25rem, 5vw, 5.25rem)", fontWeight: 760, lineHeight: 1.0 }}>Virtual Assistant Services & Business Operations Support for Growing Teams</h1><p className="mt-6 max-w-[600px] text-pretty text-base leading-8 text-black/68 lg:text-lg">SageStone Inc helps growing businesses, startups, agencies, real estate teams, and e-commerce brands delegate administrative work, customer support, web maintenance, social media coordination, and day-to-day business operations without adding payroll.</p><div className="mt-8 flex flex-col gap-3 sm:flex-row"><CtaButton location="homepage_hero">Book a Free Consultation</CtaButton><CtaButton location="homepage_hero" variant="secondary">Explore Our Services</CtaButton></div><div className="mt-7 grid max-w-xl grid-cols-3 gap-3 text-sm"><div><strong className="block text-2xl text-[color:var(--brand-charcoal)]">6</strong><span className="text-black/55">launch phases</span></div><div><strong className="block text-2xl text-[color:var(--brand-charcoal)]">8</strong><span className="text-black/55">solution teams</span></div><div><strong className="block text-2xl text-[color:var(--brand-charcoal)]">AA</strong><span className="text-black/55">accessibility mindset</span></div></div></div><OperationsDashboard /></div></section>
