...
function CtaButton({ children, location, variant = "primary", className = "" }: { children: string; location: string; variant?: "primary" | "secondary"; className?: string }) {
  ...
  const classes = `group inline-flex items-center justify-center gap-2 rounded-full px-6 py-3 text-sm font-semibold transition-all duration-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-[color:var(--brand-olive-sage)] focus-visible:ring-offset-2 ${
    isSecondary
      ? "border border-[color:var(--brand-deep-sage)]/20 bg-[color:var(--brand-cloud)]/75 text-[color:var(--brand-charcoal)] hover:bg-white hover:shadow-[0_18px_40px_rgba(35,81,59,0.10)]"
      : "bg-[color:var(--brand-deep-sage)] text-white shadow-[0_18px_46px_rgba(35,81,59,0.24)] hover:-translate-y-0.5 hover:bg-[color:var(--brand-charcoal)]"
  } ${className}`;
    ...
    <div className="hero-dashboard premium-shell relative mx-auto max-w-xl rounded-[38px] p-5 backdrop-blur-xl">
      <div className="relative overflow-hidden rounded-[30px] bg-[color:var(--brand-ink)] p-5 text-white">
        <div className="mb-5 flex items-center justify-between">
          ...
          <ShieldCheck className="h-10 w-10 text-[color:var(--brand-mint)]" />
        </div>
        <div className="grid gap-3 sm:grid-cols-2">
          {cards.map((card, i) => <div key={card} className="float-card rounded-2xl border border-white/10 bg-white/[0.07] p-4" style={{ animationDelay: `${i * 120}ms` }}><CheckCircle2 className="mb-3 h-5 w-5 text-[color:var(--brand-mint)]" /><p className="font-semibold">{card}</p><p className="mt-1 text-xs text-white/55">Assigned · tracked · optimized</p></div>)}
        </div>
        <div className="mt-5 rounded-2xl bg-white p-4 text-[color:var(--brand-charcoal)]"><div className="mb-3 flex items-center justify-between text-sm font-semibold"><span>Weekly workflow clarity</span><span>92%</span></div><div className="h-2 overflow-hidden rounded-full bg-[color:var(--brand-soft-beige)]"><div className="h-full w-[92%] rounded-full bg-[color:var(--brand-deep-sage)]" /></div></div>
      </div>
    </div>
  );
}
...
export default function Home() {
  ...
  return (
    <>
      <section className="relative isolate overflow-hidden pt-8" style={{ background: "radial-gradient(circle at 82% 18%, rgba(191,230,200,.62), transparent 26rem), linear-gradient(135deg, var(--brand-ivory), var(--brand-cloud) 50%, #edf7e9)" }}>
        <div aria-hidden="true" className="absolute inset-0 -z-10"><div className="ambient-orb absolute -right-40 top-10 h-[620px] w-[620px] rounded-full bg-[color:var(--brand-mint)]/65 blur-3xl" /><div className="ambient-orb ambient-orb-delayed absolute -left-36 bottom-0 h-[520px] w-[520px] rounded-full bg-[color:var(--brand-soft-beige)]/70 blur-3xl" /></div>
        <div className="mx-auto grid max-w-[1440px] items-center gap-12 px-4 py-16 sm:px-6 lg:grid-cols-[1.02fr_0.98fr] lg:px-8 lg:py-24">
          <div className="reveal-up">
            <p className="mb-5 inline-flex rounded-full pill-glow border border-[color:var(--brand-deep-sage)]/18 bg-[color:var(--brand-cloud)]/72 px-4 py-2 text-xs font-bold uppercase tracking-[0.2em] text-[color:var(--brand-deep-sage)]">Vetted remote operations support</p>
